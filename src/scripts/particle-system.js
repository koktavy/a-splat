...
if (typeof window.AFRAME === 'undefined') {
  throw new Error('Component attempted to register before window.AFRAME was available.');
}

window.AFRAME.registerComponent('particle-system', {
    ...
    schema: {
        ...
        type: {
            type: 'number',
            default: window.SPE.distributions.BOX
        },
        rotationAxis: {
            type: 'string',
            default: 'x'
        ...
        blending: {
            type: 'number',
            default: window.THREE.AdditiveBlending,
            oneOf: [window.THREE.NoBlending,window.THREE.NormalBlending,window.THREE.AdditiveBlending,window.THREE.SubtractiveBlending,window.THREE.MultiplyBlending]
        },
        enabled: {
            type:'boolean',
            default:true
    ...
    initParticleSystem: function(settings) {
        ...
        this.particleGroup = new window.SPE.Group({
            texture: {
        ...
        var emitter = new window.SPE.Emitter({
            maxAge: {
            ...
            position: {                
                spread: new window.THREE.Vector3(settings.positionSpread.x, settings.positionSpread.y, settings.positionSpread.z),
                randomise: settings.randomise
                //spreadClamp: new window.THREE.Vector3( 2, 2, 2 ),
                //radius: 4
            },
            rotation: {
                axis: (settings.rotationAxis=='x'?new window.THREE.Vector3(1, 0, 0):(settings.rotationAxis=='y'?new window.THREE.Vector3(0, 1, 0):(settings.rotationAxis=='z'?new window.THREE.Vector3(0, 0, 1):new window.THREE.Vector3(0, 1, 0)))),
                angle: settings.rotationAngle,
                angleSpread: settings.rotationAngleSpread,
                static: true
            },
            acceleration: {
                value: new window.THREE.Vector3(settings.accelerationValue.x, settings.accelerationValue.y, settings.accelerationValue.z),
                spread: new window.THREE.Vector3(settings.accelerationSpread.x, settings.accelerationSpread.y, settings.accelerationSpread.z)
            },
            velocity: {
                value: new window.THREE.Vector3(settings.velocityValue.x, settings.velocityValue.y, settings.velocityValue.z),
                spread: new window.THREE.Vector3(settings.velocitySpread.x, settings.velocitySpread.y, settings.velocitySpread.z)
            },
            drag: {
                value: new window.THREE.Vector3(settings.dragValue.x, settings.dragValue.y, settings.dragValue.z),
                spread: new window.THREE.Vector3(settings.dragSpread.x, settings.dragSpread.y, settings.dragSpread.z),
                randomise: settings.dragRandomise
            },
            color: {
                value: settings.color.map(function(c) { return new window.THREE.Color(c); })            
            },
