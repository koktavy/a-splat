...
/**
* A helper to handle creating and updating a window.THREE.BufferAttribute instance.
*
* @author  Luke Moody
* @constructor
...
this.bufferAttribute.usage = this.dynamicBuffer ?
  window.THREE.DynamicDrawUsage :
  window.THREE.StaticDrawUsage;
};
...
this.bufferAttribute.usage = window.THREE.StaticDrawUsage;
...
/**
* Creates a window.THREE.BufferAttribute instance if one doesn't exist already.
*
...
if ( this.bufferAttribute !== null ) {
  ...
  // Since window.THREE.js version 81, dynamic count calculation was removed
  // so I need to do it manually here.
  //
  // In the next minor release, I may well remove this check and force
  // dependency on window.THREE r81+.
  if ( parseFloat( window.THREE.REVISION ) >= 81 ) {
...
this.bufferAttribute = new window.THREE.BufferAttribute( this.typedArray.array, this.componentSize );
...
vertex: [
  ...
  window.THREE.ShaderChunk.logdepthbuf_pars_vertex,
  window.THREE.ShaderChunk.fog_pars_vertex,
  ...
  window.THREE.ShaderChunk.logdepthbuf_vertex,
  window.THREE.ShaderChunk.fog_vertex,
...
fragment: [
  ...
  window.THREE.ShaderChunk.common,
  window.THREE.ShaderChunk.fog_pars_fragment,
  window.THREE.ShaderChunk.logdepthbuf_pars_fragment,
  ...
  window.THREE.ShaderChunk.logdepthbuf_fragment,

  '    outgoingLight = vColor.xyz * rotatedTexture.xyz;',
  '    gl_FragColor = vec4( outgoingLight.xyz, rotatedTexture.w * vColor.w );',

  window.THREE.ShaderChunk.fog_fragment,
...
SPE.utils = {
  ...
  lerpTypeAgnostic: function( start, end, delta ) {
      ...
      else if ( start instanceof window.THREE.Vector2 && end instanceof window.THREE.Vector2 ) {
          out = start.clone();
          out.x = this.lerp( start.x, end.x, delta );
          out.y = this.lerp( start.y, end.y, delta );
          return out;
      }
      else if ( start instanceof window.THREE.Vector3 && end instanceof window.THREE.Vector3 ) {
          out = start.clone();
          out.x = this.lerp( start.x, end.x, delta );
          out.y = this.lerp( start.y, end.y, delta );
      ...
      else if ( start instanceof window.THREE.Vector4 && end instanceof window.THREE.Vector4 ) {
          out = start.clone();
          out.x = this.lerp( start.x, end.x, delta );
          out.y = this.lerp( start.y, end.y, delta );
          out.z = this.lerp( start.z, end.z, delta );
      ...
      else if ( start instanceof window.THREE.Color && end instanceof window.THREE.Color ) {
          out = start.clone();
   ...
   * @param  {Object} base        window.THREE.Vector3 instance describing the start value.
   * @param  {Object} spread      window.THREE.Vector3 instance describing the random variance to apply to the start value.
   * @param  {Object} spreadClamp window.THREE.Vector3 instance describing the multiples to clamp the randomness to.
   */
   ...
   * @param  {Object} attribute The instance of SPE.ShaderAttribute to save the result to.
   * @param  {Number} index     The offset in the attribute's TypedArray to save the result from.
   * @param  {Object} base      window.THREE.Color instance describing the start color.
   * @param  {Object} spread    window.THREE.Vector3 instance describing the random variance to apply to the start color.
   */
  ...
  randomColorAsHex: ( function() {
      ...
      var workingColor = new window.THREE.Color();
   ...
   * @param  {Object} attribute   The instance of SPE.ShaderAttribute to save the result to.
   * @param  {Number} index       The offset in the attribute's TypedArray to save the result from.
   * @param  {Object} start       window.THREE.Vector3 instance describing the start line position.
   * @param  {Object} end         window.THREE.Vector3 instance describing the end line position.
   */
   ...
   * @param  {Object} attribute The instance of SPE.ShaderAttribute to save the result to.
   * @param  {Number} index     The offset in the attribute's TypedArray to save the result from.
   * @param  {Object} base              window.THREE.Vector3 instance describing the origin of the transform.
   ...
   * @param  {Object} radiusScale       window.THREE.Vector3 instance describing the scale of each axis of the sphere.
   ...
   * @param  {Number} radius            The radius of the sphere to project onto.
   * @param  {Number} radiusSpread      The amount of randomness to apply to the projection result
   * @param  {Object} radiusScale       window.THREE.Vector3 instance describing the scale of each axis of the disc. The z-component is ignored.
   * @param  {Number} radiusSpreadClamp What numeric multiple the projected value should be clamped to.
   */
  ...
  randomDirectionVector3OnSphere: ( function() {
       ...
       * @param  {Object} emitterPosition window.THREE.Vector3 instance describing the emitter's base position.
  ...
  getPackedRotationAxis: ( function() {
      ...
      var v = new window.THREE.Vector3(),
          vSpread = new window.THREE.Vector3(),
          c = new window.THREE.Color(),
          addOne = new window.THREE.Vector3( 1, 1, 1 );
       ...
       * @param  {Object} axis       window.THREE.Vector3 instance describing the rotation axis.
       * @param  {Object} axisSpread window.THREE.Vector3 instance describing the amount of randomness to apply to the rotation axis.
...
*
* @property {Object} texture.value An instance of window.THREE.Texture.
*
* @property {Object=} texture.frames A window.THREE.Vector2 instance describing the number
...
SPE.Group = function( options ) {
  ...
  this.uuid = window.THREE.MathUtils.generateUUID();
  ...
  // texture stuff.
  this.texture = utils.ensureInstanceOf( options.texture.value, window.THREE.Texture, null );
  this.textureFrames = utils.ensureInstanceOf( options.texture.frames, window.THREE.Vector2, new window.THREE.Vector2( 1, 1 ) );
  ...
  this.textureFrames.max( new window.THREE.Vector2( 1, 1 ) );
  ...
  this.blending = utils.ensureTypedArg( options.blending, types.NUMBER, window.THREE.AdditiveBlending );
  ...
  this.uniforms = {
      ...
      textureAnimation: {
          type: 'v4',
          value: new window.THREE.Vector4(
      ...
      fogColor: {
          type: 'c',
          value: this.fog ? new window.THREE.Color() : null
      },
      fogNear: {
          type: 'f',
          value: 10
  ...
  this.geometry = new window.THREE.BufferGeometry();
  this.mesh = new window.THREE.Points( this.geometry, this.material );
...
SPE.Group.prototype._applyAttributesToGeometry = function() {
  ...
  // associated with a particle will be used in window.THREE's
  // render cycle.
...
SPE.Group.prototype._triggerSingleEmitter = function( pos ) {
  ...
  if ( pos instanceof window.THREE.Vector3 ) {
...
* @param  {Object} [position=undefined] A window.THREE.Vector3 instance describing the position to activate the emitter(s) at.
* @return {Group} This group instance.
*/
...
* @property {Object} [position.value=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing this emitter's base position.
* @property {Object} [position.spread=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing this emitter's position variance on a per-particle basis.
...
* @property {Object} [position.spreadClamp=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing the numeric multiples the particle's should
*                                                               be spread out over.
*                                                               Note that when using a SPHERE or DISC distribution, only the x-component
*                                                               of this vector is used.
*                                                               When using a LINE distribution, this property is ignored.
* @property {Number} [position.radius=10] This emitter's base radius.
* @property {Object} [position.radiusScale=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing the radius's scale in all three axes. Allows a SPHERE or DISC to be squashed or stretched.
...
* @property {Object} [velocity.value=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing this emitter's base velocity.
* @property {Object} [velocity.spread=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing this emitter's velocity variance on a per-particle basis.
*                                                          Note that when using a SPHERE or DISC distribution, only the x-component
*                                                          of this vector is used.
...
* @property {Object} [acceleration.value=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing this emitter's base acceleration.
* @property {Object} [acceleration.spread=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing this emitter's acceleration variance on a per-particle basis.
*                           Note that when using a SPHERE or DISC distribution, only the x-component
*                           of this vector is used.
...
* @property {Object} [rotation.axis=new window.THREE.Vector3(0, 1, 0)] A window.THREE.Vector3 instance describing this emitter's axis of rotation.
* @property {Object} [rotation.axisSpread=new window.THREE.Vector3()] A window.THREE.Vector3 instance describing the amount of variance to apply to the axis of rotation on
...
* @property {Object} [rotation.center=The value of `position.value`] A window.THREE.Vector3 instance describing the center point of rotation.
...
*                               Depending on the value of SPE.valueOverLifetimeLength, if arrays of window.THREE.Color instances are given, then the array will be interpolated to
*                               have a length matching the value of SPE.valueOverLifetimeLength.
* @property {Object} [color.value=new window.THREE.Color()] Either a single window.THREE.Color instance, or an array of window.THREE.Color instances to describe the color of a particle over it's lifetime.
* @property {Object} [color.spread=new window.THREE.Vector3()] Either a single window.THREE.Vector3 instance, or an array of window.THREE.Vector3 instances to describe the color variance of a particle over it's lifetime.
...
SPE.Emitter = function( options ) {
  ...
  this.position = {
      _value: utils.ensureInstanceOf( options.position.value, window.THREE.Vector3, new window.THREE.Vector3() ),
      _spread: utils.ensureInstanceOf( options.position.spread, window.THREE.Vector3, new window.THREE.Vector3() ),
      _spreadClamp: utils.ensureInstanceOf( options.position.spreadClamp, window.THREE.Vector3, new window.THREE.Vector3() ),
      ...
      _radiusScale: utils.ensureInstanceOf( options.position.radiusScale, window.THREE.Vector3, new window.THREE.Vector3( 1, 1, 1 ) ),
      _distributionClamp: utils.ensureTypedArg( options.position.distributionClamp, types.NUMBER, 0 ),
  };

  this.velocity = {
      _value: utils.ensureInstanceOf( options.velocity.value, window.THREE.Vector3, new window.THREE.Vector3() ),
      _spread: utils.ensureInstanceOf( options.velocity.spread, window.THREE.Vector3, new window.THREE.Vector3() ),
      _distribution: utils.ensureTypedArg( options.velocity.distribution, types.NUMBER, this.type ),
      _randomise: utils.ensureTypedArg( options.position.randomise, types.BOOLEAN, false )
  };

  this.acceleration = {
      _value: utils.ensureInstanceOf( options.acceleration.value, window.THREE.Vector3, new window.THREE.Vector3() ),
      _spread: utils.ensureInstanceOf( options.acceleration.spread, window.THREE.Vector3, new window.THREE.Vector3() ),
  ...
  this.rotation = {
      _axis: utils.ensureInstanceOf( options.rotation.axis, window.THREE.Vector3, new window.THREE.Vector3( 0.0, 1.0, 0.0 ) ),
      _axisSpread: utils.ensureInstanceOf( options.rotation.axisSpread, window.THREE.Vector3, new window.THREE.Vector3() ),
      ...
      _center: utils.ensureInstanceOf( options.rotation.center, window.THREE.Vector3, this.position._value.clone() ),
      _randomise: utils.ensureTypedArg( options.position.randomise, types.BOOLEAN, false )
  };
  ...
  this.color = {
      _value: utils.ensureArrayInstanceOf( options.color.value, window.THREE.Color, new window.THREE.Color() ),
      _spread: utils.ensureArrayInstanceOf( options.color.spread, window.THREE.Vector3, new window.THREE.Vector3() ),
      _randomise: utils.ensureTypedArg( options.position.randomise, types.BOOLEAN, false )
  };
